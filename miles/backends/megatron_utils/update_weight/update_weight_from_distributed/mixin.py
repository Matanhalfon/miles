...
class DistBucketedWeightUpdateMixin:
    ...
    def _update_lora_weights(self) -> None:
        ...
        # Breadcrumbs: the LoRA source rank transmits alone (peers wait at the gloo
        # barrier in update_weights). A hang here starves that barrier → 30-min gloo
        # timeout. These lines localize which step wedges without needing py-spy
        # (ptrace is blocked in the container).
        logger.info("[lora-sync v%s] source unload_lora_adapter start (loaded=%s)", self.weight_version, self._lora_loaded)
        if self._lora_loaded:
            ray.get(
                [engine.unload_lora_adapter.remote(lora_name=LORA_ADAPTER_NAME) for engine in self.rollout_engines]
            )
        logger.info("[lora-sync v%s] source unload done; transmit (load+broadcast) start", self.weight_version)
        self._update_lora_weight_implementation(accumulated_named_tensors)
        logger.info("[lora-sync v%s] source transmit done", self.weight_version)
